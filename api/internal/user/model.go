...
func ToPublicUser(item User) PublicUser {
	return PublicUser{
		...
		PrivacySettings:      ToPrivacySettings(item),
		NotificationSettings: ToNotificationSettings(item),
...
type NotificationSettings struct {
	...
	ActivityReminderEnabled       bool `json:"activity_reminder_enabled"`
	SystemMessageEnabled          bool `json:"system_message_enabled"`
}

func ToPrivacySettings(item User) PrivacySettings {
	return PrivacySettings{
		ShowRegion: item.IsRegionVisible(),
	}
}

func ToNotificationSettings(item User) NotificationSettings {
	return NotificationSettings{
		NotificationPermissionEnabled: item.IsNotificationPermissionEnabled(),
		ActivityReminderEnabled:       item.AllowsActivityReminderPush(),
		SystemMessageEnabled:          item.AllowsSystemMessagePush(),
	}
}

func (item User) IsRegionVisible() bool {
	return !item.HideRegion
}

func (item User) IsNotificationPermissionEnabled() bool {
	return !item.NotificationDisabled
}

func (item User) AllowsActivityReminderPush() bool {
	return item.IsNotificationPermissionEnabled()
}

func (item User) AllowsSystemMessagePush() bool {
	return item.IsNotificationPermissionEnabled()
}
...
func publicRegion(item User) string {
	if !item.IsRegionVisible() {
		return ""
	}
	return item.Region
}
