...
2. 在一个数据库事务内创建消息、更新会话摘要与未读数，并创建一条双通道投递任务。
3. 接口在事务提交后立即返回，不同步等待第三方 Push。
4. 后台工作器领取任务后调用公共双通道方法并发投递；两个通道均失败时退避重试，任一通道成功即按原通知逻辑完成本次任务。
5. Worker 复用活动与报名通知的公共双通道 `Push` 方法；极光自定义消息携带聊天数据，供应用进程存活时实时刷新。
