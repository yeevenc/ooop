...
3. 启动时立即唤醒历史待重试任务，后台单 Worker 领取后调用阿里云 `ScanImage`。
4. 所有图片场景均返回 `pass` 时，活动自动审核通过。
5. 任一场景返回 `block` 时，活动自动拒绝，命中图片、场景、标签和置信度作为拒绝原因发送给发布者。
6. 任一场景返回 `review` 且标签不是 `normal` 时，按命中规则自动拒绝；标签为 `normal` 时保留待审核，由后台人工复核。
7. 调用失败、图片下载超时、数据库临时失败时，任务按退避时间持续重试。
